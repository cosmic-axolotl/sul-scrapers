"""
Scraper para o Fort Atacadista (fortatacadista.com.br).
Plataforma: Osuper (mesma do Asun, mas com componentes de card diferentes
— não assuma que os dois usam os mesmos seletores). Detalhes e histórico
...
class FortScraper(BaseScraper):
    nome_fornecedor = "fort"
    ...
    # Elemento com o nome da loja atual no topo (ex: "Balneário Camboriu -
    # 115") — normalmente abre o modal de troca de loja ao clicar.
    SEL_ABRIR_TROCA_LOJA = "text=Você está na loja"

    # --- Card de produto no grid de categoria ---
    # CORRIGIDO (17/09, 2ª rodada): a primeira correção copiou
    # ".item-product-wrapper" do Asun por analogia (mesma plataforma
    # Osuper), mas isso deu 0 produtos de novo — confirmado com
    # debug_vazio_fort_Mercearia.html real que essa classe NÃO existe no
    # Fort. Ou seja: mesma plataforma, mas um componente de card
    # DIFERENTE (provavelmente outra versão/tema do Osuper por cliente).
    # LIÇÃO: não assumir que seletores são idênticos entre sites só por
    # compartilharem plataforma — sempre confirmar com debug_inspecionar.py
    # em cada um.
    #
    # O card de verdade do Fort fica dentro de uma div
    # "products-list-body"; cada produto é um <a> (também sem href, só
    # onClick) contendo img[alt] + preço em R$. Testado contra HTML real:
    # 28/28 cards válidos com esse seletor.
    SEL_CARD_PRODUTO = ".products-list-body a"
    SEL_NOME_PRODUTO = "img[alt]"  # nome do produto costuma vir no alt da imagem
    # AINDA NÃO CONFIRMADO -> AGORA CONFIRMADO (debug em /promocoes,
    # 17/09): SEL_PRECO_CHEIO precisou de correção — o Fort NÃO usa a
    # classe customizada "text-full-price" (essa é exclusiva do Asun,
    # mesma plataforma mas componente diferente, igual aconteceu com
    # SEL_CARD_PRODUTO). No Fort o preço riscado usa só a classe
    # genérica do Tailwind "line-through". SEL_BADGE_DESCONTO
    # ("bg-regular-promotion/15") bateu certo sem precisar de ajuste.
    # Testado contra HTML real: 24/24 produtos com desconto extraídos
    # corretamente (preco, preco_original e desconto).
    SEL_PRECO_CHEIO = ".line-through"  # preço riscado (antes do desconto)
    ...
    def definir_cep(self, cep: str) -> None:
        if (
            ...
            logger.info("Perfil persistente detectado — pulando modal de CEP (Fort).")
            return

        logger.info("Abrindo Fort Atacadista para definir CEP...")
        self.page.goto(
            "https://www.fortatacadista.com.br/", timeout=config.NAV_TIMEOUT_MS
        )
        ...
        try:
            ...
            logger.info("CEP definido com sucesso (Fort): %s", cep)
        except PlaywrightTimeoutError:
            logger.warning(
                "Modal de CEP não apareceu (Fort) — o site pode ter caído numa loja "
                "padrão que NÃO é de Porto Alegre (ex: Balneário Camboriú/SC, "
                "aconteceu aqui no teste manual). Rode setup_sessao.py e confirme "
                "manualmente a loja de POA antes de coletar de verdade."
            )
    ...
    def _extrair_precos(self, card) -> dict:
        ...
        # Aqui SIM esperamos encontrar isso com frequência (é um atacarejo
        # de verdade) — ex: "a partir de 6 unid."
        ...
        if precos_floats:
            candidatos_ativo = [
                p
                for p in precos_floats
                if resultado["preco_original"] is None
                or p != resultado["preco_original"]
            ]
            if resultado["qtd_minima_atacado"] and len(precos_floats) >= 2:
                # Card com preço de varejo E preço de atacado juntos: o
                # maior valor tende a ser o varejo, o menor o atacado
                # (mesmo padrão usado no AtacadaoScraper).
                resultado["preco"] = precos_floats[0]
                resultado["preco_atacado"] = precos_floats[-1]
            elif resultado["preco_original"] is not None:
                resultado["preco"] = (
                    candidatos_ativo[0] if candidatos_ativo else precos_floats[-1]
                )
            else:
    ...
    def _mapear_urls_por_ordem(self, soup: BeautifulSoup) -> list[Optional[str]]:
        """
        Os cards do grid não têm href — a URL de cada produto vem do
        bloco <script type="application/ld+json"> do tipo "ItemList" que
        a página injeta, com um item por produto NA MESMA ORDEM em que
        aparecem no grid (campo "position"). Confirmado assim no Asun
        (mesma plataforma Osuper); ainda não testado contra HTML real do
        Fort, mas é o comportamento esperado da própria plataforma.
        """
        import json
    ...
    def _salvar_debug_pagina_vazia(self, categoria: str) -> None:
        ...
        nome_arquivo = (
            f"debug_vazio_fort_{categoria.replace(' ', '_').replace(',', '')}.html"
        )
        caminho = os.path.join(logs_dir, nome_arquivo)
        with open(caminho, "w", encoding="utf-8") as f:
            f.write(self.page.content())
        logger.warning(
            "0 produtos em '%s' (Fort). HTML salvo em %s", categoria, caminho
        )

    def extrair_produtos_da_pagina(
        self, categoria: str, url: str, palavras_chave: Optional[list[str]] = None
    ) -> list[Produto]:
        logger.info("Coletando categoria '%s' -> %s (Fort)", categoria, url)
        ...
        try:
            ...
            for i, card in enumerate(cards):
                ...
                # URL vem do ItemList (JSON-LD), casada por posição/ordem
                # com o card atual — não existe href no <a> do card.
                ...
                if getattr(config, "SALVAR_PRINTS_ITENS", False):
                    try:
                        ...
                        caminho_print = os.path.join(
                            config.PRINTS_DIR, f"fort_{i}_{nome_seguro}.png"
                        )
                        ...
                        logger.warning(
                            "Falha ao tirar print de '%s' (Fort): %s", nome, e
                        )
            ...
            logger.info(
                "Categoria '%s' (Fort): %d produtos extraídos.",
                categoria,
                len(produtos),
            )

        except Exception:
            logger.exception("Erro ao processar categoria '%s' (Fort)", categoria)
